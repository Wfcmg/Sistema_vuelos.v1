...
type Row = { id: string; firstName: string; firstLastName: string; email: string; role: string; phone?: string; mainAddress?: string; isActive?: boolean };
const empty = (): Partial<Row> => ({ firstName: '', firstLastName: '', email: '', role: 'CUSTOMER', mainAddress: '', isActive: true });
...
@Component({ selector: 'app-admin-users', standalone: true, imports: [CommonModule, FormsModule, AdminTableComponent, AdminFormModalComponent],
  template: `
    <app-admin-table title="Usuarios" [data]="rows()" [columns]="cols" [isLoading]="loading()" [isDeleting]="saving()"
      [searchKeys]="['firstName','firstLastName','email']" (onAdd)="openCreate()" (onEdit)="openEdit($event)" (onDelete)="del($event)"/>
    <app-admin-form-modal [title]="form().id ? 'Editar Usuario' : 'Nuevo Usuario'" [open]="modal()" [isLoading]="saving()"
      ...
      <div><label class="block text-sm font-medium text-gray-700 mb-1">Apellido *</label>
        <input [(ngModel)]="form().firstLastName" required class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" /></div>
      ...
      <div><label class="block text-sm font-medium text-gray-700 mb-1">Dirección *</label>
        <input [(ngModel)]="form().mainAddress" required class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" /></div>
...
export class AdminUsersComponent implements OnInit {
  ...
  cols = [
    { key: 'name', label: 'Nombre', render: (r: Row) => `${r.firstName} ${r.firstLastName}` },
  ...
  save(e: Event) {
    ...
    const body: any = { firstName: f.firstName, firstLastName: f.firstLastName, email: f.email, role: f.role, isActive: f.isActive ?? true };
    if (f.mainAddress) body.mainAddress = f.mainAddress;
    if (f.phone) body.phone = f.phone;
    const obs = f.id ? this.svc.updateUser(f.id, body) : this.svc.createUser(body);
    obs.subscribe({ next: () => { this.modal.set(false); this.saving.set(false); this.load(); }, error: (err: any) => { console.error('Error usuario:', err); this.saving.set(false); } });
  }
