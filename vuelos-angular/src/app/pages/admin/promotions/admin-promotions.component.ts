...
type Row = { id: string; code: string; discountType: string; discountValue: number; maxUsages?: number; currentUsages?: number; isActive?: boolean };
const empty = (): Partial<Row> => ({ code: '', discountType: 'PERCENTAGE', discountValue: 0, maxUsages: undefined, isActive: true });
  ...
  template: `
    ...
    <app-admin-form-modal [title]="form().id ? 'Editar Promoción' : 'Nueva Promoción'" [open]="modal()" [isLoading]="saving()"
      ...
      <div><label class="block text-sm font-medium text-gray-700 mb-1">Máx. Usos</label>
        <input [(ngModel)]="form().maxUsages" type="number" min="0" placeholder="Sin límite" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" /></div>
...
export class AdminPromotionsComponent implements OnInit {
  ...
  cols = [
    ...
    { key: 'currentUsages', label: 'Usos', render: (r: Row) => `${r.currentUsages ?? 0}${r.maxUsages ? ' / ' + r.maxUsages : ''}` },
  ...
  save(e: Event) {
    ...
    const body: any = { code: f.code, discountType: f.discountType, discountValue: f.discountValue, isActive: f.isActive ?? true };
    if (f.maxUsages != null) body.maxUsages = f.maxUsages;
    const obs = f.id ? this.svc.updatePromotion(f.id, body) : this.svc.createPromotion(body);
