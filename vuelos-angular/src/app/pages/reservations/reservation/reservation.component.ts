  ...
  template: `
    <div class="max-w-2xl mx-auto px-4 py-8">
      ...
      <ng-container *ngIf="successData() as sd">
        <div class="flex flex-col items-center text-center py-10 space-y-6">
          <div class="w-20 h-20 rounded-full bg-green-100 flex items-center justify-center">
            <svg class="w-10 h-10 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.4" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
          </div>
          <div>
            <h1 class="text-2xl font-bold text-white">¡Pago exitoso!</h1>
            <p class="text-sm text-slate-400 mt-1">Tu reserva ha sido confirmada</p>
          </div>
          <div class="w-full bg-[#0B1020] border border-white/10 rounded-xl divide-y divide-gray-100 text-left">
            <div class="flex items-center justify-between px-5 py-3">
              <span class="text-sm text-slate-400">Código de reserva</span>
              <span class="font-mono font-bold text-amber-500">{{ sd.reservationCode }}</span>
            </div>
            <ng-container *ngIf="sd.invoice as inv">
              <div class="flex items-center justify-between px-5 py-3">
                <span class="text-sm text-slate-400">N° Factura</span>
                <span class="font-mono font-semibold text-slate-100">{{ inv.invoiceNumber }}</span>
              </div>
              <div class="flex items-center justify-between px-5 py-3">
                <span class="text-sm text-slate-400">Subtotal</span>
                <span class="text-sm text-slate-200">\${{ (+inv.subtotal).toFixed(2) }}</span>
              </div>
              <div class="flex items-center justify-between px-5 py-3">
                <span class="text-sm text-slate-400">IVA 15%</span>
                <span class="text-sm text-slate-200">\${{ (+inv.taxAmount).toFixed(2) }}</span>
              </div>
              <div class="flex items-center justify-between px-5 py-4">
                <span class="font-semibold text-slate-100">Total pagado</span>
                <span class="text-xl font-bold text-amber-500">\${{ (+inv.total).toFixed(2) }}</span>
              </div>
            </ng-container>
            <div *ngIf="!sd.invoice" class="flex items-center justify-between px-5 py-4">
              <span class="font-semibold text-slate-100">Total pagado</span>
              <span class="text-xl font-bold text-amber-500">—</span>
            </div>
          </div>
          <p class="text-xs text-slate-500">La factura fue generada automáticamente y está disponible en el detalle de tu reserva.</p>
      ...
      <button (click)="goBack()" class="text-sm text-amber-500 hover:underline mb-6 flex items-center gap-1">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.4" d="M15 19l-7-7 7-7"/></svg>
        ...
        <ng-container *ngFor="let s of steps; let i = index">
          <div class="flex items-center gap-2 flex-1">
            <div [class]="'w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 transition-colors ' + stepClass(s.n)">
              <svg *ngIf="step() > s.n" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.4" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
              <span *ngIf="step() <= s.n">{{ s.n }}</span>
            </div>
            <span [class]="'text-sm font-medium ' + (step() === s.n ? 'text-amber-600' : 'text-slate-500')">{{ s.label }}</span>
            <div *ngIf="i < steps.length - 1" [class]="'flex-1 h-0.5 mx-2 ' + (step() > s.n ? 'bg-green-400' : 'bg-[#1F2937]')"></div>
          </div>
        </ng-container>
      </div>

      <h1 class="text-2xl font-bold text-white mb-2">{{ step() === 1 ? 'Datos de pasajeros' : 'Información de pago' }}</h1>
      <p class="text-sm text-slate-400 mb-8">{{ step() === 1 ? 'Ingresa los datos de todos los pasajeros' : 'Ingresa los datos de tu tarjeta' }}</p>

      <div *ngIf="errorMsg()" class="flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl p-4 mb-6">
        <svg class="w-5 h-5 text-red-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.4" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
      ...
      <form *ngIf="step() === 1" [formGroup]="passengerForm" (ngSubmit)="nextStep()" class="space-y-6">
        <section>
          <div class="flex items-center justify-between mb-3">
            ...
            <button type="button" (click)="addPassenger()" class="text-xs text-amber-500 hover:text-amber-600 flex items-center gap-1 font-medium">
              <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.4" d="M12 4v16m8-8H4"/></svg>
              Añadir pasajero
            </button>
          </div>
            ...
            <div *ngFor="let pg of passengerArray.controls; let i = index" [formGroupName]="i"
              ...
              <div class="flex items-center justify-between mb-4">
                ...
                <button *ngIf="passengerArray.length > 1" type="button" (click)="removePassenger(i)" class="text-red-400 hover:text-red-600">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.4" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                </button>
              </div>
        ...
        <section class="bg-[#0B1020] rounded-xl border border-white/10 p-5">
          <h2 class="font-semibold text-slate-100 mb-3 flex items-center gap-2">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.4" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/></svg>
            ¿Tienes un código de descuento?
          </h2>
          <div class="flex gap-2">
            <input [(ngModel)]="promoCode" [ngModelOptions]="{standalone: true}" placeholder="Ej: VERANO25"
              class="flex-1 border border-white/15 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-amber-500 outline-none uppercase" />
          ...
          <div *ngIf="promoResult()" class="mt-3 flex items-center gap-2 text-green-700 text-sm bg-green-50 border border-green-200 rounded-lg px-3 py-2">
            <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.4" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
        ...
        <button type="submit"
          ...
          Continuar al pago
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.4" d="M9 5l7 7-7 7"/></svg>
        </button>
      </form>
      ...
      <form *ngIf="step() === 2" [formGroup]="payForm" (ngSubmit)="submitPayment()" class="space-y-6">
        ...
        <div class="bg-amber-50 border border-amber-100 rounded-xl p-4">
          <p class="text-sm font-semibold text-slate-200 mb-2">Resumen de la reserva</p>
          <div class="space-y-1">
            <div *ngFor="let p of passengerArray.value; let i = index" class="flex items-center gap-2 text-sm text-slate-300">
              <svg class="w-3.5 h-3.5 text-amber-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.4" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
              <span>{{ p.firstName }} {{ p.lastName }} — {{ p.documentNumber }}</span>
            </div>
            <div *ngIf="promoResult()" class="flex items-center gap-2 text-sm text-green-600 mt-2 pt-2 border-t border-amber-200">
              <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.4" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/></svg>
        ...
        <section class="bg-[#0B1020] rounded-xl border border-white/10 p-5 space-y-4">
          <h2 class="font-semibold text-slate-100 flex items-center gap-2">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.4" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg>
            Método de pago
          </h2>
        ...
        <div class="flex items-center gap-2 text-xs text-slate-500">
          <svg class="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.4" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
        ...
        <button type="submit" [disabled]="payLoading()"
          ...
          <svg *ngIf="!payLoading()" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.4" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
