...
const AIRPORT_FALLBACK_P50: AirportOption[] = [
  {
    iataCode: 'UIO',
    name: 'Aeropuerto Internacional Mariscal Sucre',
    cityName: 'Quito',
    countryName: 'Ecuador',
    searchText: normalizeText('UIO Quito Ecuador Aeropuerto Internacional Mariscal Sucre'),
  },
  {
    iataCode: 'SCL',
    ...
    cityName: 'Santiago',
    countryName: 'Chile',
    searchText: normalizeText('SCL Santiago Chile Aeropuerto Arturo Merino Benitez'),
  },
  {
    iataCode: 'LIM',
    ...
    cityName: 'Lima',
    countryName: 'Peru',
    searchText: normalizeText('LIM Lima Peru Aeropuerto Internacional Jorge Chavez'),
  },
];
@Component({
...
export class AirportSearchComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.svc.getAll().subscribe({
      next: data => {
        ...
        this.airports.set(mapped.length > 0 ? mapped : AIRPORT_FALLBACK_P50);
        this.loading.set(false);
        this.syncLabel();
      },
      error: () => {
        this.airports.set(AIRPORT_FALLBACK_P50);
        this.loading.set(false);
      },
    });
  }
  ...
  // FIX P49B:
  // Permite aceptar codigos IATA escritos manualmente como UIO, SCL o LIM,
  // aunque el desplegable de aeropuertos no cargue o no se seleccione.
  private acceptTypedIata(): string {
    const typed = (this.query() ?? '').trim().toUpperCase();
    const iata = typed.match(/\b[A-Z]{3}\b/)?.[0] ?? '';

    if (iata) {
      this._code = iata;
      this.valueChange.emit(iata);
      return iata;
    }
  ...
  resolveCurrentValue(): string {
    const q = normalizeText(this.query());
    const match = this.filtered()[0] ?? this.airports().find(a =>
      normalizeText(a.iataCode) === q ||
      normalizeText(a.cityName) === q ||
      normalizeText(`${a.iataCode} - ${a.cityName}`) === q
    );
