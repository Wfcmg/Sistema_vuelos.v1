...
// Repositories
const countryRepo              = new CountryRepository(prismaCatalog);
const cityRepo                 = new CityRepository(prismaCatalog);
const airportRepo              = new AirportRepository(prismaCatalog);
const airlineRepo              = new AirlineRepository(prismaCatalog);
const aircraftRepo             = new AircraftRepository(prismaCatalog);
const airlineAirportRepo       = new AirlineAirportRepository(prismaCatalog);
const airlineServiceConfigRepo = new AirlineServiceConfigRepository(prismaCatalog);
const serviceCatalogRepo       = new ServiceCatalogRepository(prismaCatalog);

// Query repos
const airportQuery              = new AirportQueryRepository(prismaCatalog);
const countryQuery              = new CountryQueryRepository(prismaCatalog);
const cityQuery                 = new CityQueryRepository(prismaCatalog);
const airlineQuery              = new AirlineQueryRepository(prismaCatalog);
const aircraftQuery             = new AircraftQueryRepository(prismaCatalog);
const airlineAirportQuery       = new AirlineAirportQueryRepository(prismaCatalog);
const airlineServiceConfigQuery = new AirlineServiceConfigQueryRepository(prismaCatalog);
const serviceCatalogQuery       = new ServiceCatalogQueryRepository(prismaCatalog);
...
async function start() {
  ...
  app.listen(PORT, () => console.log(`Ã°Å¸Å¡â‚¬ [catalog-service] Ã¢â€ â€™ http://localhost:${PORT}`));
}

process.on('SIGINT',  async () => { await prismaCatalog.$disconnect(); process.exit(0); });
process.on('SIGTERM', async () => { await prismaCatalog.$disconnect(); process.exit(0); });
process.on('uncaughtException',  (err) => { console.error('[catalog-service] ExcepciÃƒÂ³n:', err); process.exit(1); });
