...
// Repositories
const flightRepo      = new FlightRepository(prismaCatalog);
const flightClassRepo = new FlightClassRepository(prismaCatalog);
const segmentRepo     = new SegmentRepository(prismaCatalog);
const promotionRepo   = new PromotionRepository(prismaBooking);

// Query repos
const flightQuery      = new FlightQueryRepository(prismaCatalog);
const flightClassQuery = new FlightClassQueryRepository(prismaCatalog);
const segmentQuery     = new SegmentQueryRepository(prismaCatalog);
const promotionQuery   = new PromotionQueryRepository(prismaBooking);
...
async function start() {
  ...
  app.listen(PORT, () => {
    console.log(`Ã°Å¸Å¡â‚¬ [flights-service] REST Ã¢â€ â€™ http://localhost:${PORT}`);
    console.log(`Ã°Å¸â€œÂ¡ [flights-service] gRPC Ã¢â€ â€™ localhost:${GRPC_PORT}`);
  });
}

process.on('SIGINT',  async () => { await prismaCatalog.$disconnect(); await prismaBooking.$disconnect(); process.exit(0); });
process.on('SIGTERM', async () => { await prismaCatalog.$disconnect(); await prismaBooking.$disconnect(); process.exit(0); });
process.on('uncaughtException',  (err) => { console.error('[flights-service] ExcepciÃƒÂ³n:', err); process.exit(1); });
