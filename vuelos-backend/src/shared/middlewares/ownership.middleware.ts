...
import prismaBooking from '../database/prisma.booking.client.js';
import prismaCatalog from '../database/prisma.catalog.client.js';
...
async function getReservationOwner(reservationId: string) {
  return (prismaBooking as any).reservation.findUnique({
    where: { id: reservationId },
    select: { id: true, userId: true, status: true, totalAmount: true, flightId: true },
  });
}

async function assertReservationOwner(_db: PrismaClient, req: Request, res: Response, reservationId: string): Promise<boolean> {
  const reservation = await getReservationOwner(reservationId);
  if (!reservation) {
...
export function requirePassengerOwner(db: PrismaClient, getId: IdGetter) {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const id = normalizeId(getId(req));
      if (!id) return missingId(res);
      const passenger = await (db as any).reservationPassenger.findUnique({
        where: { id },
...
export function requireBillingProfileOwner(db: PrismaClient, getId: IdGetter) {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const id = normalizeId(getId(req));
      if (!id) return missingId(res);
      const profile = await (db as any).billingProfile.findUnique({ where: { id }, select: { userId: true } });
...
export function requirePaymentOwner(db: PrismaClient, getId: IdGetter) {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const id = normalizeId(getId(req));
      if (!id) return missingId(res);
      const payment = await (db as any).payment.findUnique({ where: { id } });
      if (!payment) return deny(res, 404, 'NOT_FOUND', 'Pago no encontrado');

      const reservation = await getReservationOwner(payment.reservationId);
      if (!reservation) return deny(res, 404, 'NOT_FOUND', 'Reserva del pago no encontrada');
  ...
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      if (!ownerId && invoice.payment?.reservationId) {
        const reservation = await getReservationOwner(invoice.payment.reservationId);
...
export function requirePaymentOwnerForInvoice(db: PrismaClient, getPaymentId: IdGetter) {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const payment = await (db as any).payment.findUnique({ where: { id: paymentId } });
      if (!payment) return deny(res, 404, 'NOT_FOUND', 'Pago no encontrado');
...
export function requireInvoiceItemOwner(db: PrismaClient, getId: IdGetter) {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const id = normalizeId(getId(req));
      if (!id) return missingId(res);
      const item = await (db as any).invoiceItem.findUnique({
        where: { id },
...
export function requireBoardingPassOwner(db: PrismaClient, getId: IdGetter) {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const id = normalizeId(getId(req));
      if (!id) return missingId(res);
      const boardingPass = await (db as any).boardingPass.findUnique({
        where: { id },
...
export function requireBoardingPassCreateAllowed(db: PrismaClient) {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const segment = await (prismaCatalog as any).segment.findFirst({
        where: { id: segmentId, flightId: passenger.reservation.flightId },
        select: { id: true },
      });
...
export function prepareCustomerPayment(_db: PrismaClient) {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
