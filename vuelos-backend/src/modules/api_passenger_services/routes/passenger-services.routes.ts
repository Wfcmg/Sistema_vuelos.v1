...
async function ensurePassengerServiceOwner(db: PrismaClient, req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const isAdmin = req.user?.role === 'ADMIN';
    const passengerService = await (db as any).passengerService.findUnique({
      where: { id },
...
async function lockServicePrice(_db: PrismaClient, req: Request, res: Response, next: NextFunction) {
  try {
    const serviceConfigId = String((req.body as any).serviceConfigId ?? '');
    const config = await (prismaCatalog as any).airlineServiceConfig.findUnique({ where: { id: serviceConfigId } });
    if (!config) {
