...
export class PassengerServiceRepository implements IPassengerServiceRepository {
  ...
  async findAll(page = 1, limit = 100): Promise<PagedResult<PassengerService>> {
    ...
    const [data, total] = await Promise.all([
      (this.db as any).passengerService.findMany({ skip, take: limit, include }),
  ...
  async findById(id: string): Promise<PassengerService | null> {
    return (this.db as any).passengerService.findUnique({ where: { id }, include }) as any;
  }

  async findByPassenger(passengerId: string): Promise<any[]> {
    return (this.db as any).passengerService.findMany({ where: { passengerId }, include });
  }

  async findByServiceConfig(serviceConfigId: string): Promise<any[]> {
    return (this.db as any).passengerService.findMany({ where: { serviceConfigId }, include });
  }

  async findAllWithRelations(): Promise<any[]> {
    return (this.db as any).passengerService.findMany({ include });
  }

  async create(data: any): Promise<PassengerService> {
    return (this.db as any).passengerService.create({ data, include }) as any;
  }

  async update(id: string, data: any): Promise<PassengerService> {
    return (this.db as any).passengerService.update({ where: { id }, data, include }) as any;
  }

  async delete(id: string): Promise<void> {
    await (this.db as any).passengerService.delete({ where: { id } });
  }
}
