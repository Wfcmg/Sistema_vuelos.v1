...
export class ReservationPassengerRepository implements IReservationPassengerRepository {
  ...
  async findAll(page = 1, limit = 100): Promise<PagedResult<ReservationPassenger>> {
    ...
    const [data, total] = await Promise.all([
      (this.db as any).reservationPassenger.findMany({ skip, take: limit, include }),
  ...
  async findById(id: string): Promise<ReservationPassenger | null> {
    return (this.db as any).reservationPassenger.findUnique({ where: { id }, include }) as any;
  }

  async findByReservation(reservationId: string): Promise<any[]> {
    return (this.db as any).reservationPassenger.findMany({ where: { reservationId }, include });
  }

  async findAllWithRelations(): Promise<any[]> {
    return (this.db as any).reservationPassenger.findMany({ include });
  }

  async create(data: any): Promise<ReservationPassenger> {
    return (this.db as any).reservationPassenger.create({ data, include }) as any;
  }

  async update(id: string, data: any): Promise<ReservationPassenger> {
    return (this.db as any).reservationPassenger.update({ where: { id }, data, include }) as any;
  }

  async delete(id: string): Promise<void> {
    await (this.db as any).reservationPassenger.delete({ where: { id } });
  }
}
